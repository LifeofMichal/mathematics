...
export function reverseString(value: string): string {
  ...
  if (regexWhiteSpaces.test(value))
    throw new Error(`Value "${value}" contains only white spaces`);
  if (value.length <= 1) {
    throw new Error(`Value "${value}" must be larger than 0`);
  }
  if (value) {
    // 0.98 - 1.01s - gets slower faster depending on string length
...
/**
     ...
     * The functions works based on:
        ...
        etc...
     */
export function calculateNthFibonacciNumber(n: number): number {
  let a: number = 0,
    b: number = 1,
    nthFN: number = 0,
    i: number = 3;
...
/**
     ...
     */
export function padNumberWithZeroes(n: number): string {
  ...
  while (i < lengthLimit) {
    const current = val[i];
    if (!current || regexNonNumbers.test(current)) {
      val[i] = padder;
    }
    i++;
  }
 ...
 */
export function findNthLargestNumber(
  numbers: number[],
  n: number,
): number | undefined {
  if (n < 1 || numbers.length < n)
    throw new Error(`Input must be between 1 or ${numbers.length}`);
 ...
 */
export function selectPrimeNumbers(numbers: number[]): number[] {
  let arrPrimes: number[] = [];

  for (let i = 0; i < numbers.length; i++) {
    const current = numbers[i];
    if (current && isPrime(current)) {
      arrPrimes = [...arrPrimes, current];
    }
  }
...
export function isPalindrome(value: number): boolean {
  ...
  let indexLeft: number = 0,
    indexRight: number = bitPattern.length - 1;
  ...
  while (indexLeft <= range) {
    const currentLeft = bitPattern[indexLeft];
    const currentRight = bitPattern[indexRight];
 ...
 * UTILITIES by Michal
 */
export function addOrdinalToNumber(num: number): string {
...
export function printBitPattern(num: number) {
...

{
  ...
  console.log(
    `#2 - ${number}${ordinal}(F${number - 1}) Fibonacci Number is: ${nthFN}`,
  );
}
...

{
  ...
  let arrNumbers: number[] = [
    17, 1337, 3, 12, 5, 67, 0, 23, 999, 4321, 6991, 16, 91,
  ];
  const nthLargestNumber = findNthLargestNumber(arrNumbers, number);
  console.log(
    `#5 - ${nthLargestNumber} is the ${number}${ordinal} largest number from the list - ${arrNumbers}`,
  );
}

{
  ...
  const arrNumbers: number[] = [
    1, 2, 3, 4, 5, 6, 7, 997, 999, 1337, 4321, 6991, 6992, 7919,
  ];
  const arrPrimes: number[] = selectPrimeNumbers(arrNumbers);
  console.log(
    `#6 - ${arrPrimes} are prime numbers of the list - ${arrNumbers}`,
  );
}

{
  ...
  const number = 1337;
  console.log(
    `#7 - ${number}'s bit pattern is${isPalindrome(number) ? "" : " not"} a palindrome`,
  );
}

{
  ...
  const number = 42;
  console.log(
    `#8 - Bit count for integer ${number} is ${countSetBits(number)} `,
  );
}
